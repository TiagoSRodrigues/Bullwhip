...
def Save_Configurations(actors):
    import json, sys, yaml, simulation_configuration as sc, time
    with open(sc.actors_configuration_file,"w") as file:
        file.write(yaml.dump(actors))

    with open(sc.Configuration_backups+time.strftime("actors_configuration_file "+"%Y%m%d_%H-%M-%S"+".json", time.localtime()),"w") as fp:
