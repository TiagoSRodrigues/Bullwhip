    ...
    for quantity in input_data:
        ...
        for actor in simulation.actors_collection:
            ...
            if not actor.id==0:
                    ...
                    logs.new_log(actor=actor.id, day=simulation.time, function="main", file="main", info_msg= f"present stock {actor.actor_inventory.main_inventory} ")
                    ...
                    logs.new_log(actor=actor.id, day=simulation.time, function="main", file="main", info_msg= "starting management ")
                    if not actor.manage_orders():
                        raise Exception("Error in actor orders management")

                    logs.new_log(actor=actor.id, day=simulation.time, function="main", file="main", info_msg= "starting sock management ")
